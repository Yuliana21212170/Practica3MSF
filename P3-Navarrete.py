...
Nombre del alumno: Jocelyn Yuliana Navarrete Avalos 
Número de control: 21212170
Correo institucional: l21212170@tectijuana.edu.mx
...
def plotsignals(u, sysControl, sysCaso, sysTratamiento):
    ...
    plt.plot(t, PAx, '-', color=mycolors[1], label='$F_{s}(t):Control$')

    ts, PAy = ctrl.forced_response(sysCaso, t, u, x0)
    plt.plot(t, PAy, '-', color=mycolors[2], label='$F_{s}(t):Caso$')

    ts, PAz = ctrl.forced_response(sysTratamiento, t, u, x0)
    plt.plot(t, PAz, ':', linewidth=3, color=mycolors[3], label='$F_{s}(t):Tratamiento$')

    plt.grid(False)
    plt.xlim(0, 10)
    plt.ylim(-0.1, 1.1)
    plt.xlabel('$t$ [s]', fontsize=11)
    plt.ylabel('$F_i(t)$ [V]', fontsize=11)
 ...
 fig1.set_size_inches(w, h)
    fig1.tight_layout()
    fig1.savefig('Python_SistemaMusculoesquelético_LazoCerrado.png', dpi=600, bbox_inches='tight')
    fig1.savefig('Python_SistemaMusculoesquelético_LazoCerrado.pdf', dpi=600, bbox_inches='tight')
    plt.show()
    ...
    fig2.tight_layout()
    fig2.savefig('Python_SistemaMusculoesquelético_LazoAbierto.png', dpi=600, bbox_inches='tight')
    fig2.savefig('Python_SistemaMusculoesquelético_LazoAbierto.pdf', dpi=600, bbox_inches='tight')
    plt.show()
